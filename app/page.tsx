  ...
  const [activeTab, setActiveTab] = useState<TabKey>('terms')
    ...
    return (
      <>
        <style>{`
  @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500;1,600;1,700&family=Jost:wght@300;400;500;600&family=Kapakana&family=Parisienne&family=Great+Vibes&display=swap');

  @font-face {
    font-family: 'Kapakana';
    src: url('/fonts/Kapakana-Regular.woff2') format('woff2'), url('/fonts/Kapakana-Regular.woff') format('woff');
    font-weight: 400;
    font-style: normal;
    font-display: swap;
  }       --porcelain: #FFFBF7;
  ...
  @media (min-width: 480px) {
            .folder-tab { padding: 14px 26px 12px; font-size: 1.05rem; letter-spacing: .05em; margin-left: -14px; }
          }
  ...
  .folder-body {
            ...
            z-index: 5;
            overflow: hidden;
            transition: transform .45s cubic-bezier(0.34, 1.56, 0.64, 1), opacity .35s ease, max-height .45s ease, padding .45s ease;
            transform-origin: top center;
          }
          .stationery-paper-stack.is-closed .folder-body {
            max-height: 0;
            padding-top: 0;
            padding-bottom: 0;
            opacity: 0;
          ...
          .stationery-paper-stack.is-open .folder-body {
            max-height: 3000px;
            opacity: 1;
            transform: scaleY(1);
            animation: folder-pop .5s cubic-bezier(0.34, 1.56, 0.64, 1);
          }
  ...
  .stationery-header h3 {
    ...
    color: #EA829B !important; /* 🌸 Vibrant Coquette Pink! */
    margin-bottom: 6px;
    line-height: 1.1;
  }
          .stationery-header p {
  ...
  <div className={`stationery-paper-stack ${isFolderOpen ? 'is-open' : 'is-closed'}`}>
  {!isFolderOpen && (
                  <div className="folder-closed-wrap">
                    <button
                      type="button"
                      className="folder-closed-peek coquette-hover"
                      onClick={() => setIsFolderOpen(true)}
                    >
                      <CoquetteBow width={20} height={13} style={{ color: 'var(--rose-deep)' }} />
                      <span>Open Studio Guidelines</span>
                    </button>
                  </div>
                )}
  <div className="folder-body">
                  ...
                  {isFolderOpen && (
                    <button
                      type="button"
                      className="folder-close-btn coquette-hover"
  ...
  <CoquetteBow width={48} height={32} style={{ color: '#E46888', marginBottom: 8 }} />                      <h3>Fitting Appointment</h3>
                  ...
                  {activeTab === 'how' && (
                    <div role="tabpanel" id="panel-how" aria-labelledby="tab-how" tabIndex={0} style={{ '--tab-accent': 'var(--accent-how)' } as React.CSSProperties}>
                      <div className="stationery-header">
                        <CoquetteBow width={48} height={32} style={{ color: '#EA829B', marginBottom: 8 }} />
                        <h3>How to Rent</h3>
                        <p>Everything You Need to Know</p>
                      </div>
                      <div className="stationery-grid">
