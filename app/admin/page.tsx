...
export default function AdminDashboard() {
  ...
  const updateStatus = async (bookingId: string, newStatus: string, dressId: string | null) => {
    const confirmAction = window.confirm(`Are you sure you want to mark this booking as ${newStatus}?`)
    ...
    // Release dress back to inventory if cancelled or completed
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[#8C666B] mt-1">Manage your fittings, inventory, and client requests.</p>
          </div>
          <div className="flex gap-3">
            <button 
              onClick={handleLogout}
              className="px-6 py-2 rounded-full border border-[#E5D7D3] text-[#8C666B] hover:bg-gray-50 transition-colors text-sm font-medium uppercase tracking-widest"
            >
              Sign Out
            </button>
            <Link 
                ...
                {filteredBookings.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-12 text-center">
                      ...
                      <p className="text-[#8C666B] text-sm mt-1">Try adjusting your filters.</p>
                    </td>
                  </tr>
                ) : (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {booking.status === 'pending' && (
                            <>
                              <button 
                                ...
                                className="px-4 py-2 bg-[#3D2C2E] text-white text-[10px] uppercase tracking-widest font-bold rounded-lg hover:bg-[#A9647C] transition-colors shadow-sm"
                              >
                                Approve
                              </button>
                              <button 
                                onClick={() => updateStatus(booking.id, 'cancelled', booking.dress_id)}
                                className="px-4 py-2 bg-white border border-[#E5D7D3] text-[#8C666B] text-[10px] uppercase tracking-widest font-bold rounded-lg hover:bg-red-50 hover:text-red-700 hover:border-red-200 transition-colors"
                              >
                                Reject
                              </button>
                            </>
                          ...
                          {booking.status === 'approved' && (
                            <button 
                              ...
                              className="px-4 py-2 bg-green-50 border border-green-200 text-green-700 text-[10px] uppercase tracking-widest font-bold rounded-lg hover:bg-green-100 transition-colors shadow-sm"
                            >
                              Mark Completed
                            </button>
                          )}
                           {booking.status === 'completed' && (
                            <span className="text-xs text-[#8C666B] italic font-serif">Rental fulfilled</span>
                          )}
                        </div>
                      </td>
